...
let path;
let PostgresAppRepository;
let PostgresThemeRepository;
let WiseApplication;
let ServerWiseDesktop;

if (isServer) {
  path = require('path');
  PostgresAppRepository = require('./PostgresAppRepository');
  PostgresThemeRepository = require('./PostgresThemeRepository');
...
class WiseApplicationSystem {
  constructor(options = {}) {
    ...
    if (isServer) {
      this.repository = new PostgresAppRepository(options.db || {});
      this.themeRepository = new PostgresThemeRepository(options.db || {});
    } else {
  ...
  async run() {
    ...
    const desktopResult = this.desktop.run(this.apps);
  ...
  dispatchControlEvent(appId, controlId, eventName, values = {}) {
