class WiseDesktop {
  ...
  renderWindow(application, startupResult = null) {
    ...
    if (controls.length > 0) {
      controls.forEach((control) => body.appendChild(this.renderControl(control, application.appID)));
    } else {
  ...
  // Asks each returned control's own class to patch its fresh value back
  // into the DOM (see patchElement on each system/controls/*.js class).
